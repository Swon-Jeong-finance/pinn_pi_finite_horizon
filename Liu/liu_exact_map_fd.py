...
RATIO_FIELDS = [
    ...
    "ev_w_min", "ev_w_max", "ev_x_min", "ev_x_max", "domain_factor",
...
def load_run(run_dir: Path, *, weight_dir_override: Optional[Path] = None,
    ...
    ev_w = _shrink(w_min, w_max, margin)
    ev_x = _shrink(x_min, x_max, margin)
    ...
    group_payload = {
        ...
        "horizon": problem.horizon, "eval_margin": margin,
    ...
    return RunSpec(
        ...
        market_seed=market_seed, eval_margin=margin, eval_w_bounds=ev_w,
        eval_x_bounds=ev_x, market_hash=market_hash,
...
def _variant_schedule(outer: int, verification: set[int], *, finest: int,
                      largest_domain: float, primary_boundary: str,
                      grid_factors: Sequence[int], domain_factors: Sequence[float],
                      boundaries: Sequence[str]) -> List[Tuple[int, float, str]]:
    primary = (int(finest), float(largest_domain), str(primary_boundary))
    if int(outer) not in verification:
        return [primary]
    return [
        (int(factor), float(domain), str(boundary))
        for factor in sorted(set(grid_factors))
        for domain in sorted(set(domain_factors))
...
def _assess(rows: List[Dict[str, Any]], *, key_name: str, value_name: str,
            finest: int, largest_domain: float, primary_boundary: str,
            grid_factors: Sequence[int], domain_factors: Sequence[float],
            boundaries: Sequence[str], envelope_name: str,
            abs_tolerance: float, rel_tolerance: float) -> None:
    grouped: Dict[int, List[Dict[str, Any]]] = {}
    for row in rows:
        grouped.setdefault(int(row[key_name]), []).append(row)
    for _key, group in grouped.items():
        lookup = {(int(row["grid_factor"]), float(row["domain_factor"]), str(row["boundary"])): row
                  for row in group}
        primary = lookup.get((finest, largest_domain, primary_boundary))
        if primary is None:
            continue
        ...
        if not math.isfinite(value):
            primary.update(
                ...
                boundary_abs_change=float("nan"), **{envelope_name: float("nan")},
        ...
        if not int(primary.get("is_verification", 0)):
            primary.update(grid_abs_change="", grid_rel_change="", domain_abs_change="",
                           domain_rel_change="", boundary_abs_change="",
                           **{envelope_name: ""}, refinement_status="not_checked")
            if value_name == "rho_exact":
            ...
            continue
        comparisons = {
            "grid": [lookup.get((factor, largest_domain, primary_boundary))
                     for factor in sorted(set(grid_factors)) if factor < finest],
            "domain": [lookup.get((finest, domain, primary_boundary))
                       for domain in sorted(set(domain_factors)) if domain < largest_domain],
            "boundary": [lookup.get((finest, largest_domain, boundary))
        ...
        full_candidates = [lookup.get(key) for key in expected_keys]
        complete = all(candidate is not None for candidate in full_candidates)
        complete = complete and all(
            math.isfinite(float(candidate[value_name]))
            for candidate in full_candidates if candidate is not None
        )
        full_deltas = (
        ...
        passed = complete and all(delta <= tolerance for delta in full_deltas)
        for prefix, candidates in comparisons.items():
        ...
        primary["grid_rel_change"] = changes["grid"] / max(abs(value), 1e-300)
        primary["domain_abs_change"] = changes["domain"]
        primary["domain_rel_change"] = changes["domain"] / max(abs(value), 1e-300)
        primary["boundary_abs_change"] = changes["boundary"]
        if complete:
            primary[envelope_name] = (
                value + max(full_deltas, default=0.0) if value_name == "rho_exact"
                else max(full_deltas, default=0.0)
            )
...
def evaluate_run(run: RunSpec, output: Path, *, device: str,
                 ...
                 domain_factors: Sequence[float], boundaries: Sequence[str],
    ...
    if not domain_factors or any(float(value) <= 1.0 for value in domain_factors):
        raise ValueError("domain factors must be strictly larger than one")
    if refinement_abs_tolerance < 0.0 or refinement_rel_tolerance < 0.0:
        raise ValueError("refinement tolerances must be nonnegative")
    ...
    variants_by_outer: Dict[int, Dict[Tuple[int, float, str], Tuple[Tuple[Array, ...], Dict[str, Any]]]] = {}
    input_bundles: Dict[int, Tuple[Array, ...]] = {}
    input_metrics: Dict[int, Dict[str, float]] = {}
    checkpoint_hashes: Dict[int, str] = {}
    rows: List[Dict[str, Any]] = []

    def grid_spec(factor: int, domain_factor: float) -> Tuple[FDGrid, Array, Array, Array]:
        y_center = 0.5 * (run.problem.y_min + run.problem.y_max)
        y_half = 0.5 * (run.problem.y_max - run.problem.y_min) * domain_factor
        x_center = 0.5 * (run.problem.x_min + run.problem.x_max)
        x_half = 0.5 * (run.problem.x_max - run.problem.x_min) * domain_factor
        ny = max(7, int(round(domain_factor * (base_ny - 1))) * factor + 1)
        nx = max(7, int(round(domain_factor * (base_nx - 1))) * factor + 1)
        nt = int(base_nt) * factor
        grid = FDGrid(y_center - y_half, y_center + y_half,
                      x_center - x_half, x_center + x_half, ny, nx, nt)
    ...
    for position, (outer, checkpoint) in enumerate(run.checkpoints, start=1):
        ...
        variants_by_outer[outer] = {}
        cache: Dict[Tuple[int, float], Tuple[Any, str]] = {}
        for factor, domain_factor, boundary in _variant_schedule(
            outer, verification, finest=finest, largest_domain=largest_domain,
            ...
            primary = (factor, domain_factor, boundary) == (finest, largest_domain, primary_boundary)
            row: Dict[str, Any] = {
                ...
                "eval_margin": run.eval_margin, "ev_w_min": ev_w_min, "ev_w_max": ev_w_max,
                "ev_x_min": ev_x_min, "ev_x_max": ev_x_max,
                "domain_factor": domain_factor, "fd_y_min": grid.y_min, "fd_y_max": grid.y_max,
    ...
    _assess(rows, key_name="source_outer_iter", value_name="rho_exact", finest=finest,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--domain-factors", default="1.5,2.0")
    parser.add_argument("--boundaries", default="linearity,exact-dirichlet")
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            evaluate_run(
                ...
                domain_factors=_parse_floats(args.domain_factors),
