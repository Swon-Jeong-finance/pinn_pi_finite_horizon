# Liu affine finite-domain PI-map and E4 diagnostic
...
solve. The default object is a **finite-domain boundary-projected policy
extension map**. It is not the whole-space exact PI map assumed in the theorem,
and neither the file names nor the shorthand `rho_exact` should be used to
remove that qualification in paper text.
...
iterate must have a checkpoint. When E4 is requested, an exploratory
`--checkpoints` selection must therefore be a contiguous prefix `1,...,k`.
An arbitrary sparse selection is permitted only with `--skip-e4`; that mode
computes exact-map ratios only and is explicitly ineligible for paper
aggregation.
...
Every sparse time-step solve is also subject to the hard normalized-residual
gate

\[
\frac{\|A z-b\|_\infty}{\max(1,\|b\|_\infty)}
\leq \texttt{--linear-residual-tolerance},
\]

whose default is \(10^{-8}\). A variant that violates this bound fails
immediately; a completed CSV is not evidence that the gate passed unless it
was generated by the current implementation. Before time stepping, the
boundary-elimination block must have full rank and infinity-norm condition
number no larger than `--boundary-condition-limit` (default \(10^{12}\)).
The output additionally records the smallest LU diagonal-pivot ratio observed
over time. The corresponding CSV fields are `max_linear_residual`,
`boundary_elimination_size`, `boundary_elimination_rank`,
`boundary_elimination_cond_inf`, and
...
The enlarged rectangle extends beyond the saved nominal collocation bounds.
The default `--policy-extension boundary-projection` therefore defines

\[
\vartheta_{\mathrm{ext}}(\tau,y,x)
=
\vartheta_{\mathrm{NN}}\!\left(
  \tau,
  \Pi_{[y_{\min},y_{\max}]}y,
  \Pi_{[x_{\min},x_{\max}]}x
\right).
...
Thus the normalized feedback is held at its nearest collocation-boundary value
outside the rectangle on which the network was trained. The FD state
coefficients, including \(\lambda(x)\), continue to use the actual FD state;
only the neural policy query is projected. This convention avoids treating
uncontrolled neural extrapolation as an identified policy, but it defines a
finite-domain extension map rather than a whole-space exact map. Rows are
labelled
`map_definition=finite_domain_boundary_projected_policy_extension` and
`whole_space_map_claim=not_a_whole_space_map`.

`--policy-extension neural-extrapolation` retains the unprojected neural
feedback only as an exploratory sensitivity calculation. The extension mode,
saved nominal collocation bounds, and the fractions of FD points lying outside
those bounds are recorded in the protocol and CSV. Do not pool projected and
raw-extrapolation results.
...
feedback is sampled only on a finite enlarged rectangle. Under the default
boundary projection, “unmodified” refers only to the absence of the concavity
guard and control clipping on the projected policy support; it does not turn
the projected extension into the ideal whole-space greedy operator.
`local_greedy_unmodified_on_policy_support` should be read together with
`policy_extension`, `map_definition`, and `whole_space_map_claim`.
...
## Sparse domain-factor pilot

Before committing to an expensive all-checkpoint protocol, evaluate the five
representative source iterates \(k\in\{1,5,10,15,20\}\) over the proposed
enlargements:

```bash
python3 Liu/liu_exact_map_fd.py \
  --run-dir /path/to/liu_main/pipinn_N30_M1_seed1 \
  --output /path/to/derived/liu_fd_domain_pilot_seed1 \
  --device cuda:0 \
...
`--skip-e4` is essential for a non-contiguous checkpoint subset. The resulting
`analysis_mode=sparse_exact_map_only_pilot`,
`paper_aggregation_eligible=false`, and header-only E4 CSVs are intentional.
The pilot output must not be supplied to `aggregate_liu_exact_map.py`.

Do not select a domain merely because it gives the smallest
`rho_exact`. A candidate must first satisfy all of the following:

- zero `guard_frac_fd`, `theta_any_clip_frac_fd`, and
  `theta_component_clip_frac_fd`;
- strictly positive sampled ellipticity with a comfortable margin and zero
  `nonpositive_log_eig_fraction`;
- a full-rank boundary-elimination block with acceptable condition number,
  a nondegenerate LU-pivot ratio, and
  `max_linear_residual <= linear_residual_tolerance`;
- small changes under grid refinement, neighboring domain factors, and the
  alternative boundary closure.

Only after these gates pass should the magnitude and stability of
`rho_exact` be compared. Freeze the selected domain/grid/boundary protocol and
then rerun the complete `1,...,outer_iters` schedule with E4 enabled.

The evaluator stages a requested Cartesian run as one transaction. Therefore
one failed factor or boundary prevents a partial comparison table from being
committed. To isolate failures during diagnosis, run one domain factor per
separate output directory, for example:

```bash
for d in 1.05 1.10 1.25 1.50; do
  python3 Liu/liu_exact_map_fd.py \
    --run-dir /path/to/liu_main/pipinn_N30_M1_seed1 \
    --output "/path/to/derived/liu_fd_domain_${d}_seed1" \
    --device cuda:0 \
    --checkpoints 1,5,10,15,20 \
    --skip-e4 \
    --grid-factors 1,2 \
    ...
    --verify-checkpoints all \
    --policy-extension boundary-projection \
    --overwrite || printf 'failed domain factor: %s\n' "$d"
done
```

These isolated runs are diagnostic only. A failed variant must remain visible
as a failure; do not silently aggregate only the successful factors.
...
Current CSVs populate the component fields
`e_input_value`, `e_input_vw`, `e_input_vww`, `e_input_vwx`,
`e_input_bundle` and their `e_map_*`/`e_approx_*` counterparts. Older outputs
created before the component-schema correction may contain blank component
columns even when `rho_exact` is present. Those blanks cannot be reconstructed
reliably by renaming columns after the fact: rerun `liu_exact_map_fd.py` from
the checkpoints, using a new output directory or `--overwrite`, and verify the
new artifact hashes before aggregation.
